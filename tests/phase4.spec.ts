...
test('phase 4 backward parity by layer + tiny e2e input gradient', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const fixture = await loadJson<Phase4Fixture>('/phase4-backprop/phase4_backprop_fixture.json')
    ...
    const reluBackward = await ask({ type: 'tensor-op', id: 'p4-relu', op: 'relu-backward', input: { shape: relu.input, values: relu.inputValues }, gradOut: { shape: relu.input, values: relu.gradOutValues } })
    const pool = fixture.pool
    const poolBackward = await ask({ type: 'tensor-op', id: 'p4-pool', op: 'maxpool2d-backward', input: { shape: pool.input, values: pool.inputValues }, gradOut: { shape: pool.gradOut, values: pool.gradOutValues } })
    const norm = fixture.normalize
    const normBackward = await ask({ type: 'tensor-op', id: 'p4-norm', op: 'normalize-backward', input: { shape: norm.shape, values: norm.inputValues }, gradOut: { shape: norm.shape, values: norm.gradOutValues }, std: norm.std })
    const conv = fixture.convBackwardInput
    const convBackward = await ask({ type: 'tensor-op', id: 'p4-conv', op: 'conv2d-backward-input', inputShape: conv.inputShape, gradOut: { shape: conv.gradOutShape, values: conv.gradOutValues }, weight: { shape: conv.weightShape, values: conv.weightValues } })
  ...
  for (const [response, expected] of checks) {
    ...
    if (response.type !== 'tensor-op-result' || !response.ok || !('values' in response)) throw new Error('bad response')
    expected.forEach((value, index) => expect(response.values[index]).toBeCloseTo(value, 4))
  }
})
