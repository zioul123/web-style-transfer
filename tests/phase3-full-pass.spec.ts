...
test('phase 3 full vgg19 pass parity through conv5_1 style + conv4_2 content losses', async ({ page }) => {
  ...
  const result = await page.evaluate(async ({ reluLayers, poolLayers }) => {
    ...
    const normalizedContent = await ask({ type: 'tensor-op', id: 'phase3-full-norm-content', op: 'normalize-forward', input: { shape: fixture.inputShape, values: fixture.contentImageValues }, mean: fixture.mean, std: fixture.std })
    ...
    let currentContentShape: [number, number, number, number] = fixture.inputShape
    ...
    for (let layerIndex = 0; layerIndex <= 29; layerIndex += 1) {
      ...
      if (Array.isArray(weightValues) && Array.isArray(biasValues) && Array.isArray(weightShape)) {
        const convContent = await ask({ type: 'tensor-op', id: `conv-c-${layerIndex}`, op: 'conv2d-forward', input: { shape: currentContentShape, values: currentContentValues }, weight: { shape: weightShape as [number, number, number, number], values: weightValues }, bias: biasValues })
        ...
        currentContentShape = [1, weightShape[0], currentContentShape[2], currentContentShape[3]]
      ...
      if (poolLayers.includes(layerIndex)) {
        ...
        currentContentShape = [1, currentContentShape[1], Math.floor(currentContentShape[2] / 2), Math.floor(currentContentShape[3] / 2)]
