...
function App() {
  ...
  const onUpload = async (event: React.ChangeEvent<HTMLInputElement>, target: 'content' | 'style'): Promise<void> => {
    ...
    if (target === 'content') {
      setContentImage(previewUrl); setContentTensor(tensor); setInputTensor(tensor); setOutputImage(tensorValuesToDataUrl(tensor, resolution))
    } else {
