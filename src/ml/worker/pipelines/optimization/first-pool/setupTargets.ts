...
export const setupFirstPoolTargets = async (device: GPUDevice, payload: Extract<WorkerRequest, { type: "run-first-pool-optimizer" }>): Promise<FirstPoolPersistentContext> => {
  ...
  try {
    const relu1Shape = convOutputShape(payload.inputShape, conv1Weight.shape[0]);
    ...
    styleRelu2Buffer = await runConv2dReluForwardBuffer(styleRelu1Buffer, relu1Shape, conv2Weight.values, conv2Weight.shape, payload.conv2Bias);
    stylePoolBuffer = await runMaxPool2dForwardBuffer(styleRelu2Buffer, relu2Shape);
    styleRelu3Buffer = await runConv2dReluForwardBuffer(stylePoolBuffer, poolShape, conv3Weight.values, conv3Weight.shape, payload.conv3Bias);
    ...
    contentRelu2Buffer = await runConv2dReluForwardBuffer(contentRelu1Buffer, relu1Shape, conv2Weight.values, conv2Weight.shape, payload.conv2Bias);
    const styleGram1Buffer = await runGramMatrixBuffer(styleRelu1Buffer, relu1Shape);
    const styleGram3Buffer = await runGramMatrixBuffer(styleRelu3Buffer, relu3Shape);
    const contentRelu2TargetBuffer = uploadToOwnedBuffer(device, await readGpuBufferToArray(device, contentRelu2Buffer.buffer, elementCount(relu2Shape)));
