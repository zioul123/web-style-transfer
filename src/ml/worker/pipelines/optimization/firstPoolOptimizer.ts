...
): number => shape[0] * shape[1] * shape[2] * shape[3];
...
export const runFirstPoolOptimizer = async (
  ...
  const stats: FirstPoolStatsAccumulator = {
    forwardMs: 0,
    lossMs: 0,
    backwardMs: 0,
    updateMs: 0,
    readbackMs: 0,
    mandatoryReadbackMs: 0,
    diagnosticsReadbackMs: 0,
  };
  const collectBenchmarkStats = payload.collectBenchmarkStats ?? false;
  const losses: number[] = [];
  let inputBuffer = uploadToOwnedBuffer(
    device,
  ...
  try {
    for (let step = 0; step < payload.steps; step += 1) {
      const result = await runFirstPoolStep(
        device,
        payload,
        persistent,
        inputBuffer,
