...
export const makeFusedUpdateClampVec4Shader = (vecCount: number, learningRate: number): string => `
...
export const runFusedUpdateClampBuffer = (
  ...
  buffer: useVec4 && count % 4 === 0
    ...
    : runUnaryShaderToBuffer(device, makeFusedUpdateClampShader(count, learningRate), input.buffer, count, [
      { binding: 1, resource: { buffer: grad.buffer } },
    ]),
  owned: true,
});
