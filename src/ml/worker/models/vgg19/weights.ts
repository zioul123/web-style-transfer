...
const expectedByteLength = (
  ...
  dtype: ManifestTensorEntry["dtype"],
): number =>
...
const decodeWeights = (
  ...
  if (entry.dtype === "int4-packed") {
    if (entry.quantization === undefined)
      throw new Error("Missing quantization metadata for int4 tensor.");
    if (entry.quantization.packing !== "int4x2-lsn-first") {
      throw new Error(
        `Unsupported int4 packing scheme: ${String(entry.quantization.packing)}.`,
      );
    }
    ...
    if (entry.quantization.scale.length !== channelCount)
      throw new Error("Invalid int4 quantization scale length.");
  ...
  if (entry.quantization === undefined)
    throw new Error("Missing quantization metadata for int8 tensor.");
  const channelCount = entry.shape[entry.quantization.axis];
  if (entry.quantization.scale.length !== channelCount)
    throw new Error("Invalid int8 quantization scale length.");
  const channelStride = tensorElemCount(entry.shape.slice(1));
...
export const parseVgg19ManifestBackedLayerCache = async (
  ...
  if (manifest.checksums.algorithm !== "sha256")
  ...
  for (const shardMeta of manifest.shards) {
    ...
    if (buffer === undefined)
      throw new Error(`Missing shard buffer: ${shardMeta.name}`);
    if (buffer.byteLength !== shardMeta.byteLength) {
      throw new Error(
        `Shard ${shardMeta.name} length mismatch: ${buffer.byteLength} vs ${shardMeta.byteLength}.`,
      );
    }
    ...
    if (expected !== undefined) {
      ...
      if (actual !== expected)
        throw new Error(
          `Checksum validation failed for shard ${shardMeta.name}.`,
        );
    }
  }
  ...
  for (
    ...
    if (weight.length !== weightExpected)
      throw new Error(`conv${layerIndex} weight byte length mismatch.`);
    if (bias.length !== biasExpected)
      throw new Error(`conv${layerIndex} bias byte length mismatch.`);
    const weightShard = shardBuffers[weight.shard];
    const biasShard = shardBuffers[bias.shard];
    if (weightShard === undefined || biasShard === undefined)
      throw new Error(`Missing shard for conv${layerIndex}.`);
    if (weight.offset + weight.length > weightShard.byteLength)
      throw new Error(`conv${layerIndex} weight range out of bounds.`);
    if (bias.offset + bias.length > biasShard.byteLength)
      throw new Error(`conv${layerIndex} bias range out of bounds.`);

    const weightValues = decodeWeights(weight, weightShard);
    const biasValues = new Float32Array(
      biasShard.slice(bias.offset, bias.offset + bias.length),
    );
    if (!isTensorShape4D(weight.shape))
      throw new Error(`conv${layerIndex} weight shape invalid.`);
    if (bias.shape.length !== 1 || !Number.isInteger(bias.shape[0]))
      throw new Error(`conv${layerIndex} bias shape invalid.`);
    if (biasValues.length !== bias.shape[0])
      throw new Error(`conv${layerIndex} bias shape/value mismatch.`);
    cache[layerIndex] = {
      shape: weight.shape,
      values: weightValues,
      bias: biasValues,
