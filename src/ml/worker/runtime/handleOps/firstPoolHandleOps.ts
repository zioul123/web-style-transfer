...
export const normalizeForwardHandle = async (input: RuntimeTensorHandle, shape: RuntimeTensorShape, mean: readonly [number, number, number], std: readonly [number, number, number]): Promise<RuntimeTensorHandle> =>
  toHandle(shape, await runNormalizeForward(getGpuDevice(), runUnary, await fromHandle(input), shape, mean, std, BUFFER_USAGE_STORAGE_COPY_DST, BUFFER_USAGE_UNIFORM_COPY_DST));

export const conv2dForwardHandle = async (input: RuntimeTensorHandle, inputShape: RuntimeTensorShape, weightValues: Float32Array, weightShape: readonly [number, number, number, number], bias: number[], outputShape: RuntimeTensorShape): Promise<RuntimeTensorHandle> =>
  toHandle(outputShape, await runConv2dForward(getGpuDevice(), runUnary, await fromHandle(input), inputShape, weightValues, weightShape, bias, BUFFER_USAGE_STORAGE_COPY_DST, BUFFER_USAGE_UNIFORM_COPY_DST));
...
export const styleLossBackwardHandleFromTarget = async (input: RuntimeTensorHandle, inputShape: RuntimeTensorShape, target: RuntimeTensorHandle): Promise<RuntimeTensorHandle> =>
  toHandle(inputShape, await runStyleLossBackward(getGpuDevice(), runUnary, await fromHandle(input), inputShape, await fromHandle(target), BUFFER_USAGE_STORAGE_COPY_DST, BUFFER_USAGE_UNIFORM_COPY_DST));
export const contentLossBackwardHandle = async (input: RuntimeTensorHandle, target: RuntimeTensorHandle, shape: RuntimeTensorShape): Promise<RuntimeTensorHandle> => toHandle(shape, await runContentLossBackward(getGpuDevice(), runUnary, await fromHandle(input), await fromHandle(target), BUFFER_USAGE_STORAGE_COPY_DST));
export const reluBackwardHandle = async (forwardInput: RuntimeTensorHandle, gradOutput: RuntimeTensorHandle, shape: RuntimeTensorShape): Promise<RuntimeTensorHandle> => toHandle(shape, await runReluBackward(getGpuDevice(), runUnary, await fromHandle(forwardInput), await fromHandle(gradOutput)));
export const maxPoolBackwardHandle = async (forwardInput: RuntimeTensorHandle, inputShape: RuntimeTensorShape, gradOutput: RuntimeTensorHandle): Promise<RuntimeTensorHandle> => toHandle(inputShape, await runMaxPool2dBackward(getGpuDevice(), runUnary, await fromHandle(forwardInput), inputShape, await fromHandle(gradOutput), BUFFER_USAGE_STORAGE_COPY_DST, BUFFER_USAGE_UNIFORM_COPY_DST));
export const conv2dBackwardInputHandle = async (inputShape: RuntimeTensorShape, gradOutput: RuntimeTensorHandle, weightValues: Float32Array, weightShape: readonly [number, number, number, number]): Promise<RuntimeTensorHandle> => toHandle(inputShape, await runConv2dBackwardInput(getGpuDevice(), runUnary, inputShape, await fromHandle(gradOutput), weightValues, weightShape, BUFFER_USAGE_STORAGE_COPY_DST, BUFFER_USAGE_UNIFORM_COPY_DST));
export const normalizeBackwardHandle = async (gradOutput: RuntimeTensorHandle, inputShape: RuntimeTensorShape, std: readonly [number, number, number]): Promise<RuntimeTensorHandle> => toHandle(inputShape, await runNormalizeBackward(getGpuDevice(), runUnary, await fromHandle(gradOutput), inputShape, std, BUFFER_USAGE_STORAGE_COPY_DST, BUFFER_USAGE_UNIFORM_COPY_DST));
