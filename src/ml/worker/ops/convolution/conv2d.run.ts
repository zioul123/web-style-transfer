import { makeConv2dBackwardInputShader, makeConv2dReluShader, makeConv2dShader } from './conv2d.shader'
...
export const runConv2dBackwardInput = async (gpuDevice: GPUDevice | null, runUnaryShader: (code: string, input: Float32Array, outCount: number, extraEntries?: GPUBindGroupEntry[]) => Promise<Float32Array>, inputShape: readonly [number, number, number, number], gradOut: Float32Array, weight: Float32Array, weightShape: readonly [number, number, number, number], BUFFER_USAGE_STORAGE_COPY_DST: number, BUFFER_USAGE_UNIFORM_COPY_DST: number): Promise<Float32Array> => {
