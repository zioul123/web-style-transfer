...
def _load_pack_model(pack: str) -> nn.Sequential:
    ...
    for layer_index in range(0, LAST_LAYER_INDEX + 1):
        ...
        if w['dtype'] == 'float32':
            ...
            linear = (q - zp) * scale
            if quant['scheme'] == 'per-channel-symmetric-log':
                w_t = torch.sign(linear) * torch.expm1(linear.abs())
            else:
                w_t = linear
...
def main() -> None:
    ...
    for pack in ('fp16', 'int8-per-channel', 'int8log-per-channel', 'int4-experimental', 'int4log-experimental'):
